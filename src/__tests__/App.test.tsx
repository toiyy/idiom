...
describe('学習記録', () => {
  function recordItems(): HTMLElement[] {
    return Array.from(document.querySelectorAll('.record-item'));
  }
  ...
  it('押した時点までに解いたぶんが 1 件になる', async () => {
    const user = userEvent.setup();
    render(<App />);
    await user.click(screen.getByRole('button', { name: SHORT.name }));
    await answer(user, { correct: true, confidence: '自信あり' });
    ...
    await user.click(screen.getByRole('button', { name: 'ここまでを記録する' }));
    expect(screen.getByRole('heading', { name: '記録（1 件）' })).toBeInTheDocument();
    expect(recordItems()[0]).toHaveTextContent('前回から 2 問');
    expect(recordItems()[0]).toHaveTextContent('1 / 2');
    expect(recordItems()[0]).toHaveTextContent('自信あり 1/1');
  });

  it('2 回目は前回からの差だけが出る', async () => {
    const user = userEvent.setup();
    render(<App />);
    await user.click(screen.getByRole('button', { name: SHORT.name }));
    await answer(user, { correct: true, confidence: '自信あり' });
    ...
    const [latest, first] = recordItems();
    expect(latest).toHaveTextContent('前回から 1 問');
    expect(latest).toHaveTextContent('0 / 1');
    expect(first).toHaveTextContent('前回から 2 問');
  });

  it('複数カテゴリをまたいで解いても 1 件にまとまる', async () => {
    const user = userEvent.setup();
    render(<App />);
    for (const name of [SHORT.name, /^時制/, /^比較/]) {
    ...
    expect(recordItems()[0]).toHaveTextContent('前回から 3 問');
    expect(recordItems()[0]).toHaveTextContent('3 / 3');
  });

  it('要復習の減り具合が出る', async () => {
    const user = userEvent.setup();
    render(<App />);
    await user.click(screen.getByRole('button', { name: SHORT.name }));
    const missed = currentQuestion();
    ...
    await user.click(screen.getByRole('button', { name: '中断' }));
    await user.click(screen.getByRole('button', { name: 'ここまでを記録する' }));
    expect(recordItems()[0]).toHaveTextContent('要復習 1');
    ...
    expect(recordItems()[0]).toHaveTextContent('要復習 0');
    expect(recordItems()[0]).toHaveTextContent('−1');
  });
  ...
  it('書き出しに記録が含まれ、取り込むと復元される', async () => {
    ...
    await user.click(screen.getByRole('button', { name: '書き出す' }));
    const json = (document.querySelector('.transfer__area') as HTMLTextAreaElement).value;
    expect(parseBackup(json)?.records).toHaveLength(1);
