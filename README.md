...
現在の同梱データ: **21 カテゴリ / 402 問**（すべて Part 5 形式）。各カテゴリ 8 問以上。
...
現在 402 問。反復で答えを覚えてしまわないための目安である 280 問（1 日 20 問
...
そこで論点の濃い 6 カテゴリ（時制・態・仮定法・助動詞・代名詞・主述の一致）を
各 15 問に揃えた（+34 問）。語彙 + 品詞識別の割合は 40.8% → **37.3%**。

残りのフラット 10 カテゴリ（各 8〜10 問）も 15 問に揃えれば、割合はさらに
32% 程度まで下がり、各カテゴリを 2 分割できるだけの厚みにもなる。
